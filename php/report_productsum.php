...
if($numr>0){
   $link="year=".$_GET["year"]."&month=".$_GET["month"]."&data=".$_GET["data"];
   header("Location: report_productsum_re.php?$link");
  // echo ("<script>window.location='report_productsum_re.php?".$link."'</script>");
}
...
print "<tr><td style='text-align: center;border:1px solid #a2a2a2;'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_a))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_b))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_c))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_d))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_e))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_f))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_g))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_h))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_i))."' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='-' class='cal_sum'></td>".
      "<td style='text-align: right;border:1px solid #a2a2a2;'><input type='text' value='".recheck_num(array_sum($sum_j))."' class='cal_sum'></td>";
...
<div>
  <button class='btn' onclick="save_stock_m()">บันทึกข้อมูล</button>
...
$objQuery = mysql_query($strSQL)or die(mysql_error());
  ...
  $sqls = "SELECT group_type from stock_product_es WHERE dateday = '".$_POST["year"]."-".$_POST["month"]."-30' ";
  $num = mysql_num_rows(Mysql_Query($sqls));
  // list($nobill_system) = Mysql_fetch_row(Mysql_Query($sqls));
if(empty($num)){
$sql_px = "SELECT * FROM stock_product WHERE pcs > '0'";
$result_px = mysql_query($sql_px);
while($ps = mysql_fetch_array($result_px)){

$strSQL = "INSERT INTO stock_product_es SET "; 
$strSQL .="dateday = '".$_POST["year"]."-".$_POST["month"]."-30' ";
$strSQL .=",group_type = '".$ps["group_type"]."' ";
$strSQL .=",barcode = '".$ps["barcode"]."' ";
$strSQL .=",pcs_stock = '".$ps["pcs"]."' ";
$strSQL .=",price = '".$ps["price_in"]."' ";
$objQuery = mysql_query($strSQL) or die(mysql_error());
