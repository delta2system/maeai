...
<head>
  ...
  <script src="../js/jquery-1.8.0.min.js"></script>
	...
	function return_detail(){
            ...
            $.ajax({ 
                url: "mysql_hirefix_type.php" ,
                type: "POST",
                data: 'submit=return_hirefix_type&search='+$("input[name=search]").val(),
            })
             ...
             //alert(result);
              ...
              if(obj != ''){
                $.each(obj, function(key, val) {
                  ...
                  tr = tr +  "<td class='border_bt'>&nbsp;&nbsp;"+val["type_name"]+"</td>";
		...
		function edit_d(rx){
		 $.ajax({ 
                url: "mysql_hirefix_type.php" , type: "POST", data: 'submit=return_hirefix_type&row_id='+rx,
            })
            .success(function(result) {
            	//alert(result);
            var obj = jQuery.parseJSON(result);
           if(obj != ''){$.each(obj, function(key, val) {
            ...
            $("input[name=edit_detail]").val(val["type_name"]);
		...
		function add_personal(){
			...
			$.ajax({ 
                url: "mysql_hirefix_type.php" ,
                type: "POST",
                data: 'submit=return_hirefix_type_no',
            })
            .success(function(result) {
		...
		function save_customer(){
			var data = "&row_id="+$("input[name=edit_code]").val();
				data = data + "&type_name=" + $("input[name=edit_detail]").val();

          $.ajax({ 
                url: "mysql_hirefix_type.php" ,
                type: "POST",
                data: 'submit=save_hirefix_type'+data,
            })
            .success(function(result) {
            	//alert(result);
       //        var obj = jQuery.parseJSON(result);
		...
		function del_customer(){
			...
			var data = "&row_id="+$("input[name=edit_code]").val();
   			...
   			$.ajax({ 
                url: "mysql_hirefix_type.php" ,
                type: "POST",
                data: 'submit=del_hirefix_type'+data,
            })
            .success(function(result) {
            	$("#display_edit").hide();
           		$('#display_user').show();
...
<body>
<div style="width:100%;height:100px;left:0px;top:0px;background-color:#66ff66;"><br><br>
	<div class="head_bar"> <i class="fa fa-braille"></i>  | เพิ่ม/แก้ไข ประเภทงานซ่อม</div>
...
<div id="display_edit" style="display:none;width:70%;height:400px;background-color: #ffffff;position: absolute;top:120px;right: 0px;">
	...
	<div style="margin-top:20px;color:#3385ff; "> รายการ : <input type="text" name="edit_detail" style="width:285px;font-size: 15px;padding: 5px 5px;color:#555555;"></div>
