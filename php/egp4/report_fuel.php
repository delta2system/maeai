...
<table style="width:21cm;margin:0px auto;">
  <thead>
  	<tr><td colspan="15" style="text-align: center;font-size: 18px;font-weight: bold;">รายงานน้ำมัน ประจำเดือน 
      <select name="month" style="font-size: 17px;border:0px solid #000000;" onchange="return_dataall()">
      <?
      for ($i=1; $i <= 12 ; $i++) { 
        if(date("m")==str_pad($i,2,STR_PAD_LEFT)){
      print "<option selected value='".str_pad($i, 2,'0',STR_PAD_LEFT)."'>".thai_month(str_pad($i, 2,'0',STR_PAD_LEFT))."</option>";    
        }else{
...
<div id="recive_fuel" style="margin-top: 100px;">
  <table id="rev_data" style="width:21cm;margin:0px auto;">
    <thead>
    <td class="topbar" colspan="6" style="text-align: center;border:1px solid #909090;">ยอดน้ำมันประจำเดือน <span id="month2"></span></td>
    ...
    <td class="topbar" style="border:1px solid #909090;">หน่วย</td>
    ...
    <tfoot>
      <td><input type="date" name="dateday" style='width:90%;' value="<?=date("Y-m-d")?>" onchange="year_thai(this)"></td>
      ...
      <td><select name="unit"  style="font-size: 16px;width:100%">
        <option>ลิตร</option>
