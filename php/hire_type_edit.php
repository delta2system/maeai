...
<head>
  ...
  <script src="../js/jquery-1.8.0.min.js"></script>
	...
	function return_detail(){
            ...
            $.ajax({ 
                url: "mysql_hire_type.php" ,
                type: "POST",
                data: 'submit=return_hire_type&search='+$("input[name=search]").val(),
            })
            .success(function(result) {
              ...
              if(obj != ''){
                $.each(obj, function(key, val) {
                  ...
                  tr = tr +  "<td class='border_bt'>&nbsp;&nbsp;"+val["material"]+"</td>";
		...
		function edit_d(rx){
		 $.ajax({ 
                url: "mysql_hire_type.php" ,
                type: "POST",
                data: 'submit=return_hire_type&row_id='+rx,
            })
            .success(function(result) {
            	//alert(result);
            var obj = jQuery.parseJSON(result);
           if(obj != ''){$.each(obj, function(key, val) {
           	...
           	$("select[name=edit_material]").val(val["material_type"]);
		...
		function add_personal(){
			...
			$.ajax({ 
                url: "mysql_hire_type.php" ,
                type: "POST",
                data: 'submit=return_hire_type_no',
            })
            .success(function(result) {
		...
		function save_customer(){
			var data = "&row_id="+$("input[name=edit_code]").val();
				...
				data = data + "&material_type=" + $("select[name=edit_material]").val();


          $.ajax({ 
                url: "mysql_hire_type.php" ,
                type: "POST",
                data: 'submit=save_hire_type'+data,
            })
            .success(function(result) {
            	//alert(result);
       //        var obj = jQuery.parseJSON(result);
		...
		function del_customer(){
			...
			var data = "&row_id="+$("input[name=edit_code]").val();
   			...
   			$.ajax({ 
                url: "mysql_hire_type.php" ,
                type: "POST",
                data: 'submit=del_hire_type'+data,
            })
            .success(function(result) {
            	$("#display_edit").hide();
           		$('#display_user').show();
...
<div style="width:100%;height:100px;left:0px;top:0px;background-color:#00ffbf;"><br><br>
	<div class="head_bar"> <i class="fa fa-braille"></i>  | เพิ่ม/แก้ไข หมวดคลังนอก</div>
...
<div id="display_user" style="margin:0px auto;width:100%;height:400px;">
	<div>
			<table style="width:98%;" id="detail_table">
				<thead >
					<td class="top_bar">รหัส</td>
					<td class="top_bar">ประเภท</td>
					<td class="top_bar">หมวด</td>
          			<td class="top_bar"><input type="text" name="search" onkeyup="return_detail()" style="font-size:15px;width:120px;border:1px solid #2eb82e;border-radius: 5px;"><i class="fa fa-search" style="color:#a2a2a2;margin-left: -20px;"></i></td>
				</thead>
				<tbody>
...
<div id="display_edit" style="display:none;width:70%;height:400px;background-color: #ffffff;position: absolute;top:120px;right: 0px;">
	...
	<div style="margin-top:20px;color:#3385ff; "> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ประเภท : 
		<!-- <input type="text" name="edit_material" style="width:285px;font-size: 15px;padding: 5px 5px;color:#555555;"> -->
		<select name="edit_material" style="width:285px;font-size: 15px;padding: 5px 5px;color:#555555;">
			<option value=""></option>
			<?
$sql = "SELECT * from material_type ORDER By row_id  ASC";
$result = mysql_query($sql);
while ($row = mysql_fetch_array($result) ) {
	print "<option value='$row[row_id]'>$row[detail]</option>";
}
			?>
		</select>

	</div>
	<div style="margin-top:20px;color:#3385ff; "> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;หมวด : <input type="text" name="edit_detail" style="width:285px;font-size: 15px;padding: 5px 5px;color:#555555;"></div>
