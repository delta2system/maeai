...
if($_GET["nobill"]){
  ...
  $strSQL = "SELECT * FROM  tbl_import2_head where row_bill = '".$_GET["nobill"]."' limit 1 ";
  $objQuery = mysql_query($strSQL) or die (mysql_error());
  $intNumField = mysql_num_fields($objQuery);
  ...
  function addCommas(nStr)
     ...
     function search_theorder(ex){
      ...
      $.ajax({ 
        ...
        function billorder(rd){
            window.location='import_verify2.php?nonumber='+rd;
        }
      ...
      $.ajax({ 
                url: "return_detail.php" ,
                type: "POST",
                data: 'xTable=search_import2&data='+ex,
            })
            .success(function(result) { 
                //alert(result);
                var obj = jQuery.parseJSON(result);
                          ...
                          $.each(obj, function(key, val) {
                            r++;
                                   ...
                                   var tr = "<tr class='link_data' onclick=\"billimport('"+val["row_bill"]+"')\">";
                                    tr = tr +  "<td style='text-align:center;' class='td_border'>"+r+"</td>";
                                    tr = tr +  "<td style='text-align:center;' class='td_border'>"+val["dateday"]+"</td>";
                                    tr = tr +  "<td style='text-align:center;' class='td_border'>"+val["daterecipt"]+"</td>";
            ...
            window.location='import_verify2.php?nobill='+rd;
        }
    ...
    <body>
        ...
        <table style="width:680px;margin:0px auto;">
            <tr><td style="text-align: center;font-weight: bold;font-size: 26px;color:#0066ff;" colspan="2">รายงานผลการตรวจรับพัสดุคลังนอก</td></tr>
            ...
            <tr><td colspan="2">ที่ได้สั่งซื้อจาก..........................................................................................................................................................................<span style="position: absolute;width:580px;margin-left: -580px;text-align: center;"><nl style="background-color: #ffffff;"><input type="text" name="company" style="width:90%;" value="<?=$arrCol[company]?>"></nl></span></td></tr>
            <tr><td style="height:10px;" colspan="2">เลขที่ใบเสร็จ/ใบส่งของ <input type="text" name="nobill_recipt" value="<?=$arrCol[nobill_recipt]?>" style="width:80px;"> หมวด 
              <!-- <input type="text" name="type_hire" value="<?=$arrCol[type_hire]?>">  -->
              <select  name="type_hire" style="width:180px;">
              <?
              $sql_limit = "SELECT * from hire_type ";
              $result_limit = mysql_query($sql_limit) or die(mysql_error());
               while( $limit = mysql_fetch_array($result_limit)){
                if($limit[row_id]==$arrCol[type_hire]){$ck="selected";}else{$ck="";}
                print "<option $ck value='$limit[row_id]'>$limit[detail]</option>";
              }
            ...
            <select  name="department" style="width:160px;">
              <?
              $sql_limit = "SELECT * from department ";
              $result_limit = mysql_query($sql_limit) or die(mysql_error());
               while( $limit = mysql_fetch_array($result_limit)){
                if($limit[code]==$arrCol[department]){$ck="selected";}else{$ck="";}
                print "<option $ck value='$limit[code]'>$limit[name]</option>";
              }
            ...
            {
                $t++;
                print "<td style=\"text-align: left;\">$t.........................................................................................<span style=\"width:300px;position: absolute;margin-left:-300px;text-align: left;\"><nl  style=\"background-color: #ffffff;\"><input type=\"text\" name=\"detail[]\" style=\"width:100%;\" value='$data[detail] จำนวน $data[pcs] $data[unit]'></nl></span></td>";
              ...
              $strSQL = "SELECT * FROM  tbl_import2_body where row_bill = '".$_GET["nobill"]."'  ";
              $objQuery = mysql_query($strSQL) or die (mysql_error());
             while($data = mysql_fetch_array($objQuery))
            {
                $t++;
                print "<td style=\"text-align: left;\">$t.........................................................................................<span style=\"width:300px;position: absolute;margin-left:-300px;text-align: left;\"><nl  style=\"background-color: #ffffff;\"><input type='hidden' name='row_id[]' value='$data[row_id]'><input type=\"text\" name=\"detail[]\" style=\"width:100%;\" value='$data[detail] จำนวน $data[pcs] $data[unit]'></nl></span></td>";
...
<div style="width:100%;height:72%;overflow: auto;">
<table id="show_import" style='margin: 0px auto;width:90%;'>
    <thead>
...
if(empty($_POST["row_bill"]) && $_POST){
  ...
  $sql = "SELECT row_bill from tbl_import2_head ORDER By row_bill DESC  limit 1  ";
  ...
  $strSQL .=",type_hire= '".$_POST["type_hire"]."'"; 
  ...
  for ($i=0; $i < count($_POST["detail"]); $i++) { 
   ...
   $ps=explode(" ", $de[1]);
  $strSQL = "INSERT INTO tbl_import2_body SET "; 
  $strSQL .="row_bill= '".($row_bill+1)."'"; 
  $strSQL .=",detail = '".$de[0]."'"; 
  $strSQL .=",pcs= '".$ps[1]."'"; 
  $strSQL .=",unit= '".$ps[2]."'"; 
...
echo ("<script> window.location='import_verify2_print.php?row_bill=".($row_bill+1)."'</script>");
}else if($_POST["row_bill"]){

  //$strSQL = "INSERT INTO tbl_import2_head SET "; 
  $strSQL = "UPDATE tbl_import2_head SET ";
  $strSQL .="nobill_location = '".$_POST["nobill_location1"]."/".$_POST["nobill_location2"]."'"; 
  $strSQL .=",dateday= '".$_POST["today3"]."-".$_POST["today2"]."-".$_POST["today1"]."'"; 
  $strSQL .=",title= '".$_POST["title"]."'"; 
  $strSQL .=",pcs_recipt= '".$_POST["pcs_recipt"]."'"; 
  ...
  //$strSQL = "INSERT INTO tbl_import2_body SET "; 
  $strSQL = "UPDATE tbl_import2_body SET ";
  $strSQL .="detail = '".$de[0]."'"; 
  $strSQL .=",pcs= '".$ps[1]."'"; 
  $strSQL .=",unit= '".$ps[2]."'"; 
  ...
  $sql_del = "DELETE FROM tbl_import2_body WHERE row_id = '".$_POST["row_id"][$i]."'"; 
...
echo ("<script> window.location='import_verify2_print.php?row_bill=".$_POST["row_bill"]."'</script>");
}
 ?>
