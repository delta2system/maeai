...
<td style="text-align: center;border:1px solid #a2a2a2;" rowspan="2">รายงานวัสดุใช้ไป<br><?=thai_month($_GET["month1"])?> <?=$_GET["year1"]?> ถึง <?=thai_month($_GET["month2"])?> <?=$_GET["year2"]?></td>
<?
...
$sum_x=array("");
$sum_y=array("");
$sql = "SELECT * from department where 1 ORDER By code  ASC";
...
// $strSQL = "SELECT sum(total_money) from tbl_import2_head where type_hire='$hire_type' AND department = '$row[code]' AND dateday like '".$_GET["year"]."-".$_GET["month"]."%'  ";
  $strSQL = "SELECT sum(total_money) from tbl_import2_head where type_hire='$hire_type' AND department = '$row[code]' AND dateday BETWEEN '".$_GET["year1"]."-".$_GET["month1"]."-01' AND '".$_GET["year2"]."-".$_GET["month2"]."-31'  ";
...
$strSQL2="SELECT SUM(price*pcs) FROM bill  WHERE status = 'OWH' AND customer_id = '$row[code]' AND dateday BETWEEN '".$_GET["year1"]."-".$_GET["month1"]."-01' AND '".$_GET["year2"]."-".$_GET["month2"]."-31' AND (".$search.")  ";
list($sum_OWH) = Mysql_fetch_row(Mysql_Query($strSQL2));



echo "<td style='text-align: right;border:1px solid #a2a2a2;'><a href='report_department.php?dateday=01-".$_GET["month1"]."-".$_GET["year1"]."&today=31-".$_GET["month2"]."-".$_GET["year2"]."&department=".$row["code"]."&group_type=".$rowx[$t]."' target='_blank'>".recheck_num($total_outstore+$sum_OWH)."</a>&nbsp;&nbsp;</td>";
$sum_x[$i]=$sum_x[$i]+($total_outstore+$sum_OWH);
$sum_y[$t]=$sum_y[$t]+($total_outstore+$sum_OWH);
}
