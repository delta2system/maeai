...
$q="SELECT * FROM stock_product $search ORDER By barcode ASC ";
$qr=mysql_query($q);
$row_num=mysql_num_rows($qr);
$col_arr=array("กลุ่มสินค้า","รหัส","รายการ","จำนวน","หน่วยนับ","ราคา");
...
header("Content-Disposition: attachment;filename=stock_warehouse.xls "); 
?>
...
while($rs=mysql_fetch_array($qr)){
            $sql = "SELECT detail from group_type where code = '".$rs['group_type']."'  limit 1  ";
          list($group_name) = Mysql_fetch_row(Mysql_Query($sql));
?>   
   <Row ss:Height="15">
    <!-- <Cell><Data ss:Type="Number"><?=$rs['barcode']?></Data></Cell> -->
    <Cell ss:StyleID="s78"><Data ss:Type="String"><?=$group_name?></Data></Cell>
